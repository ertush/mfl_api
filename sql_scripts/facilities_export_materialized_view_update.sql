-- Drop materialized view facilities_excel_export
-- Create materialized view facilities_excel_export
...
SELECT facilities_facility.id,
       ...
       ARRAY(SELECT DISTINCT facilities_service.name
        ...
        ARRAY(SELECT DISTINCT facilities_facilityinfrastructure.infrastructure_id
             FROM facilities_facilityinfrastructure
             WHERE facilities_facilityinfrastructure.facility_id = facilities_facility.id) AS infrastructure,
        ARRAY(SELECT DISTINCT facilities_infrastructure.name
             FROM facilities_infrastructure
                      JOIN facilities_facilityinfrastructure
                          ON facilities_infrastructure.id = facilities_facilityinfrastructure.infrastructure_id
                WHERE facilities_facilityinfrastructure.facility_id = facilities_facility.id) AS infrastructure_names,
        ARRAY(SELECT DISTINCT facilities_infrastructurecategory.name
             FROM facilities_facilityinfrastructure
                      JOIN facilities_infrastructurecategory
                                  ON facilities_infrastructurecategory.id = facilities_facilityinfrastructure.infrastructure_id
                WHERE facilities_facilityinfrastructure.facility_id = facilities_facility.id) AS infrastructure_categories,
        ARRAY(SELECT DISTINCT facilities_facilityspecialist.speciality_id
             FROM facilities_facilityspecialist
             WHERE facilities_facilityspecialist.facility_id = facilities_facility.id) AS speciality,
        ARRAY(SELECT DISTINCT facilities_speciality.name
             FROM facilities_speciality
                      JOIN facilities_facilityspecialist
                                ON facilities_speciality.id = facilities_facilityspecialist.speciality_id
            WHERE facilities_facilityspecialist.facility_id = facilities_facility.id) AS speciality_names,
        ARRAY(SELECT DISTINCT facilities_specialitycategory.name
                  FROM facilities_facilityspecialist
        JOIN facilities_speciality ON facilities_facilityspecialist.speciality_id = facilities_speciality.id
        JOIN facilities_specialitycategory ON facilities_speciality.category_id = facilities_specialitycategory.id
        WHERE facilities_facilityspecialist.facility_id = facilities_facility.id) AS speciality_categories
...
FROM facilities_facility
         ...
         LEFT JOIN facilities_facilityadmissionstatus
                   ON facilities_facilityadmissionstatus.id = facilities_facility.admission_status_id
