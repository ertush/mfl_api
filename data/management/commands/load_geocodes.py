...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        facility_file = 'data/new_data/geocodes/facility_codes.json'
            ...
            for record in data[0].get('records'):
                ...
                if latitude and longitude:
                    ...
                    try:
                        ...
                        continue
                    try:
                        method = GeoCodeMethod.objects.get(
                            name=record.get('method'))
                    except:
                        raise
                    try:
                        source = GeoCodeSource.objects.get(
                            name=record.get('source'))
                    except:
                        source = GeoCodeSource.objects.create(
                            name=record.get('source'), created_by=system_user,
